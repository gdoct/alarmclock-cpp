...
void AlarmClockView::clear() { 
    display->clear(); 
}

void AlarmClockView::write(const std::string &text) { 
    display->write(text); 
}

void AlarmClockView::update(std::unique_ptr<AlarmClockState> model, Time &machine_time) {
    ...
    if (model->mode == AlarmClockModes::ALARM_ACTIVE) {
        flash_display = false;
    }
...
void AlarmClockView::set_brightness(const float &brightness) {
